...
public class PerfectFailureDetector implements IFailureDetector {
    ...
    public PerfectFailureDetector(Process p){
		this.p = p;
		t = new Timer();
		suspectedProcesses = Collections.synchronizedList(new ArrayList<Integer>(Utils.MAX_NUM_OF_PROCESSES));
        successfulReplies  = Collections.synchronizedList(new ArrayList<Integer>(Utils.MAX_NUM_OF_PROCESSES));
    }
    ...
    public void receive(Message m){
        ...
        if (lastHeartbeat + 150 > System.currentTimeMillis()){
            //valid ping reply..
    ...
    class PeriodicTask extends TimerTask{
		public void run(){
            Utils.out(p.pid,"Beat :: " + System.currentTimeMillis());
